FORNECEDOR =  """SELECT distinct p.id_pessoa   as ID_PESSOA, 
                            CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj,14,'0') end) As VARCHAR(50)) as ID_FORNECEDOR, 
                            CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj,14,'0') end) As VARCHAR(50)) as NU_CNPJ_CPF, 
...
SERVIDOR = """SELECT LPAD(num_cpf, 11,'0') as ID_SERVIDOR,
...
FORNECEDOR_COMPLEMENTO= """SELECT f.id_fornecedor as ID_FORN_SICAF,
                        ...
                        LPAD(num_cpf, 11,'0') as CPF_RESPONSAVEL,
...
FORNECEDOR_SOCIO = """Select DISTINCT CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as ID_FORNECEDOR,
       p.nom_nome as NO_FORNECEDOR,
       p.id_fornecedor_proprietario as ID_FORNECEDOR_PROPRIETARIO,
       p.tp_vinculo_fornecedor as TP_VINCULO_FORNECEDOR,
       CAST((case when i.tp_pessoa = 1 then LPAD(i.num_cpf_cnpj, 11,'0') when i.tp_pessoa = 2 then LPAD(i.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as SOCIO
  from pessoa p
inner join PESSOA i on p.id_fornecedor_proprietario = i.id_fornecedor_proprietario 
Where p.tp_vinculo_fornecedor in(1,8) 
and p.data_hora_desvinculacao is null and i.data_hora_desvinculacao is null;
"""
SOCIO = """ Select DISTINCT 
socio,
id_socio,
no_socio,
pai_socio,
...
from (
Select
    CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as socio,
    CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as id_socio,
    p.nom_nome as no_socio,    
    CAST(case when (p.id_fornecedor_proprietario <>  p.id_pessoa) then CAST(p.id_fornecedor_proprietario As VARCHAR(50)) end As VARCHAR(50))  as pai_socio,
...
Select
	CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as socio,
	CAST(i.id_pessoa As VARCHAR(50)) as id_socio,      
    ...
    p.num_participacao_acionaria
from pessoa p
inner join fornecedor_juridico j on p.id_fornecedor_proprietario = j.id_fornecedor_juridico
inner join pessoa i on i.id_pessoa = j.id_pessoa_juridica
Where p.data_hora_desvinculacao is null and i.data_hora_desvinculacao is null and p.id_pessoa <> p.id_fornecedor_proprietario
union
Select
       CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as socio,
       LPAD(p.num_cpf_cnpj, 11,'0') as id_socio,
       c.nom_nome as no_socio,
       CAST(case when (i.id_fornecedor_proprietario <>  p.id_pessoa) then CAST(i.id_fornecedor_proprietario As VARCHAR(50)) end As VARCHAR(50))  as pai_socio,
	   'Conjuge' as grupo,
	   p.tp_pessoa as tp_socio,
	   p.num_participacao_acionaria
from conjuge c
inner join pessoa p on p.id_pessoa = c.id_pessoa_fisica and p.tp_vinculo_fornecedor not in (1, 8)
inner join fornecedor_fisico f on p.id_fornecedor_proprietario = f.id_fornecedor_fisico
inner join pessoa i on i.id_pessoa = f.id_pessoa_fisica and p.tp_vinculo_fornecedor not in (1, 8)
Where c.data_hora_desvinculacao is null
union
Select
       CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as socio,
       LPAD(p.num_cpf_cnpj, 11,'0') as id_socio,
       i.nom_nome as no_socio,
	   CAST(j.id_pessoa_juridica As VARCHAR(50)) as pai_socio,
	   'Fornecedor do Conjuge' as grupo,
	   '2' as tp_socio,
	   p.num_participacao_acionaria
from conjuge c
inner join pessoa p on p.id_pessoa = c.id_pessoa_fisica
inner join fornecedor_juridico j on p.id_fornecedor_proprietario = j.id_fornecedor_juridico
inner join pessoa i on i.id_pessoa = j.id_pessoa_juridica
Where c.data_hora_desvinculacao is null and i.data_hora_desvinculacao is null
) s;
"""

SOCIO_BCK = """Select DISTINCT
    CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as socio,
    CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as id_socio,
    p.nom_nome as no_socio,
...
Select DISTINCT
       ...
       LPAD(p.num_cpf_cnpj, 11,'0') as id_socio,
       c.nom_nome as no_socio,
       CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as pai_socio,
	   'Conjuge' as grupo,
	   p.tp_pessoa as tp_socio,
	   p.num_participacao_acionaria
from conjuge c
...
Select DISTINCT
       CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as socio,
       CAST(j.id_pessoa_juridica As VARCHAR(50))  as id_socio,
       i.nom_nome as no_socio,
	  LPAD(p.num_cpf_cnpj, 11,'0') as pai_socio,
	   'Fornecedor do Conjuge' as grupo,
	   p.tp_pessoa as tp_socio,
	   p.num_participacao_acionaria
from conjuge c
...
FORNECEDOR_PENALIDADE = """SELECT
	 case when f.tp_pessoa = 1 then LPAD(f.num_cpf_cnpj, 11,'0') when f.tp_pessoa = 2 then LPAD(f.num_cpf_cnpj, 14,'0') else F.num_cpf_cnpj end as ID_FORNECEDOR,
      'SICAF' as ORIGEM,
 ...
 LEFT JOIN motivo_ocorrencia_dinamica M ON OS.id_motivo = M.id_motivo_ocorrencia_dinamica
 LEFT JOIN uasg U ON U.id_uasg = O.id_uasg
union 
SELECT
      case when f.tp_pessoa = 1 then LPAD(f.num_cpf_cnpj, 11,'0') when f.tp_pessoa = 2 then LPAD(f.num_cpf_cnpj, 14,'0') else F.num_cpf_cnpj end as ID_FORNECEDOR,
      'SICAF' as ORIGEM,
...
union
SELECT
	 case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') else p.num_cpf_cnpj end as ID_FORNECEDOR,
      'SICAF' as ORIGEM,
      'Inidoneidade' as TIPO,
	 OS.dt_inicial AS DT_INICIO,
	 OS.dt_final AS DT_FINAL,
	 ...
	 O.dsc_detalhe AS DETALHE
 FROM ocorrencia O
 inner JOIN fornecedor_juridico f ON f.id_fornecedor_juridico = O.id_fornecedor
 inner join pessoa p on p.id_pessoa = f.id_pessoa_juridica
 INNER JOIN ocorrencia_inidoneidade OS ON OS.id_ocorrencia_inidoneidade = O.id_ocorrencia 
 LEFT JOIN motivo_ocorrencia_dinamica M ON OS.id_motivo = M.id_motivo_ocorrencia_dinamica
 LEFT JOIN uasg U ON U.id_uasg = O.id_uasg
union 
...
SELECT
	 ...
	 O.dsc_detalhe AS DETALHE
 FROM ocorrencia O
 INNER JOIN fornecedor_fisico f ON f.id_fornecedor_fisico = O.id_fornecedor_infrator
 INNER join pessoa p on p.id_pessoa = f.id_pessoa_fisica
 INNER JOIN ocorrencia_inidoneidade OS ON OS.id_ocorrencia_inidoneidade = O.id_ocorrencia 
 LEFT JOIN motivo_ocorrencia_dinamica M ON OS.id_motivo = M.id_motivo_ocorrencia_dinamica
 LEFT JOIN uasg U ON U.id_uasg = O.id_uasg
union 
SELECT
	 ...
	 O.dsc_detalhe AS DETALHE
 FROM ocorrencia O
 INNER JOIN fornecedor_fisico f ON f.id_fornecedor_fisico = O.id_fornecedor
 INNER join pessoa p on p.id_pessoa = f.id_pessoa_fisica
 INNER JOIN ocorrencia_impedimento OS ON OS.id_ocorrencia_impedimento = O.id_ocorrencia 
 LEFT JOIN motivo_ocorrencia_dinamica M ON OS.id_motivo = M.id_motivo_ocorrencia_dinamica 
 LEFT JOIN uasg U ON U.id_uasg = O.id_uasg
...
Select t.id_fornecedor AS ID_FORNECEDOR,
       '5' AS ORDEM,
	   'Regularidade Fiscal e Trabalhista Federal: ' AS ATRIBUTO,
		case when
...
from (
Select CAST((case when p.tp_pessoa = 1 then LPAD(p.num_cpf_cnpj, 11,'0') when p.tp_pessoa = 2 then LPAD(p.num_cpf_cnpj, 14,'0') end) As VARCHAR(50)) as id_fornecedor, 
...
Select t.id_fornecedor AS ID_FORNECEDOR,
       '6' AS ORDEM,
      'Regularidade Fiscal Estadual/Distrital e Municipal: ' AS ATRIBUTO,
case when	
